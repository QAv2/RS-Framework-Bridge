...
## Relationship to subsequent cold-rederivation work

This package is the **January 12 2026 publication artifact** — the prior pass, generated under Anthropic Claude Opus 4.5. It is preserved here as a self-contained, redistributable record of that work.

A **cold re-derivation** of the same problem was performed three and a half months later (April 2026) under Anthropic Claude Opus 4.7, with this package quarantined behind a methodology seal until the cold pass was committed in writing. See `../../cold-derivation/` (two directories up):
...
- `04-experiment-validation.md` — re-runs `experiments/experiment_08_continuum_equations.py` from this package, identifies the first-principles value of γ as the central open question (`gamma` is hard-coded throughout these experiments — e.g. `experiment_08` line 142, γ = 0.01)

The cold pass converged with this package on the central reading (RS2 reframes the Millennium Problem; cubic damping `-γω³` dominates quadratic stretching; the rotation → `g = -ω²` → self-damping mechanism). It extended with explicit unit-scale grounding (ω_max = 1/t₀); this prior pass extended with the quaternion-derived `g = -|q_imag|²` and the eight-experiment computational chain in `experiments/`.

A **γ-derivation supplement** at `../../supplement/gamma-derivation-supplement.md` then attempts a first-principles derivation of γ from the molecular self-damping mechanism plus the atomic-zone S³ phase-volume normalization, yielding γ ≈ t₀ / (2π²) ≈ 7.7 × 10⁻¹⁸ s as a Level-2 estimate under stated assumptions, with three alternative normalizations within ~20× spread.

Both subsequent works are part of a seven-instance cross-version cold re-derivation cycle documented in the parent repository's methodology paper (currently local-only, pre-arXiv).
